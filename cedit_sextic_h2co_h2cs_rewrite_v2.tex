...
In the stronger structural interpretation developed above, the same
five-dimensional sextic sector can be viewed as canonically equivalent to
\[
\mathcal H_6^{(\mathrm{even})}\oplus r^6\mathcal H_0,
\]
that is, to one isotropic amplitude and four amplitudes spanning the even
harmonic sextic sector.  The lower harmonic sectors
$r^2\mathcal H_4^{(\mathrm{even})}$ and
$r^4\mathcal H_2^{(\mathrm{even})}$ do not provide additional independent
coordinates; rather, they are fixed by the graph relation defining the
physical sextic subspace.  The coordinates $\boldsymbol\sigma$ reported below
therefore furnish the operational representation of an underlying $1+4$
harmonic structure.  In addition, under the cyclic representation changes
\[
I^r \rightarrow II^r \rightarrow III^r \rightarrow I^r,
\]
the same physical sector admits a representation-adapted decomposition
\[
1\oplus 1\oplus 1\oplus 2,
\]
that is, three scalar directions and one two-dimensional doublet that mixes
...
representation-independent sextic content.  At the same time, the structural
interpretation of the same subspace as both a $1+4$ harmonic sector and a
representation-adapted $1\oplus1\oplus1\oplus2$ sector shows that the present
sextic model is more than a convenient numerical reduction: it identifies a
canonical physical sector inside the full even sextic operator space and
clarifies how its coordinates behave under cyclic changes of principal-axis
representation.  In practical terms, this means that sextic parameter sets
obtained in different representations may be converted to the same canonical
coordinates, compared together with their residual norms and transformation
condition numbers, and only then used as input for any final refit in the
representation judged to be numerically most favourable.
