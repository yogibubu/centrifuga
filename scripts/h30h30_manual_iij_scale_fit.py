#!/usr/bin/env python3
"""Fit a scalar normalization for the manual H30H30 iij pivot."""
...
def main() -> None:
    ...
    for species, refs in VCI_PAPER.items():
        ...
        post = residual - alpha * pivot
        print(
            f"{species:>4}  alpha={alpha: .6e}  "
            f"||res||/||delta||={np.linalg.norm(residual)/np.linalg.norm(delta): .6e}  "
    ...
    all_pivot = np.concatenate(all_pivot)
    alpha = float(np.dot(all_pivot, all_residuals) / max(np.dot(all_pivot, all_pivot), 1.0e-30))
    post = all_residuals - alpha * all_pivot
    print()
    print(
        f"global alpha={alpha: .6e}  "
        f"||post||/||res||={np.linalg.norm(post)/np.linalg.norm(all_residuals): .6e}"
    )
