#!/usr/bin/env python3
"""Audit residual directions after removing the best manual-iij projection."""
...
def main() -> None:
    for species, refs in VCI_PAPER.items():
        ...
        for name in COMPARE_NAMES:
            vec = _watson_vector(species, name)
            cos = float(np.dot(post, vec) / max(np.linalg.norm(post) * np.linalg.norm(vec), 1.0e-30))
            print(f"  {name:>30}  cos(post,{name})={cos: .6e}")
        print()
