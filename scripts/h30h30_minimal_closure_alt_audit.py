#!/usr/bin/env python3
"""Alternative minimal H30H30 closure audit with a mixed water-like branch."""
...
def _species_matrix(species: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    mat = np.column_stack([vec(name) for name in BASIS])
    target = np.array([VCI_PAPER[species][k] for k in WATSON_KEYS], dtype=float) - trusted
    return mat, target
...
def main() -> None:
    ...
    print("H30H30 alternative minimal closure audit")
    print("basis =", BASIS)
    print("global coefficients =", dict(zip(BASIS, coeffs)))
    print("rank =", rank)
    ...
    for species, (mat, target) in cache.items():
        fit = mat @ coeffs
        print(f"\n{species.upper()}")
        print(f"  ||target-fit||/||target|| = {np.linalg.norm(target-fit)/max(np.linalg.norm(target),1.0e-30):.6g}")
        coeffs_species, *_ = np.linalg.lstsq(mat, target, rcond=None)
        post = target - mat @ coeffs_species
        print(f"  species-local residual      = {np.linalg.norm(post):.6g}")
        print(f"  species-local coeffs        = {dict(zip(BASIS, coeffs_species))}")
