#!/usr/bin/env python3
"""Fit separate pure/mixed scalings for the manual H30H30 iij pivot."""
...
def main() -> None:
    ...
    for species, refs in VCI_PAPER.items():
        ...
        a = np.zeros((len(WATSON_KEYS), 2), dtype=float)
        a[list(PURE_IDX), 0] = pivot[list(PURE_IDX)]
        a[list(MIX_IDX), 1] = pivot[list(MIX_IDX)]
        coeffs, *_ = np.linalg.lstsq(a, residual, rcond=None)
        post = residual - a @ coeffs
        print(
            f"{species:>4}  alpha_pure={coeffs[0]: .6e}  alpha_mix={coeffs[1]: .6e}  "
            f"||post||/||delta||={np.linalg.norm(post)/np.linalg.norm(delta): .6e}"
        )
        all_a.append(a)
    ...
    print()
    print(
        f"global alpha_pure={coeffs[0]: .6e}  alpha_mix={coeffs[1]: .6e}  "
        f"||post||/||res||={np.linalg.norm(post)/np.linalg.norm(all_b): .6e}"
    )
