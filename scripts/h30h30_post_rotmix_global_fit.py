#!/usr/bin/env python3
"""Fit the residual after diag0+rotmix in the span of existing follow-up scaffolds."""
...
def main() -> None:
    ...
    for species in ("h2o", "h2s", "h2co", "h2cs"):
        ...
        base = vec("diag_0_iii_iii") + vec("diag_1_iii_iii_rotmix_candidate")
        target = np.array([VCI_PAPER[species][key] for key in WATSON_KEYS], dtype=float)
        residual = target - base
        candmat = np.column_stack([vec(name) for name in FOLLOWUP_CANDIDATES])
        cache[species] = (base, target, residual, candmat)
        for i in range(len(WATSON_KEYS)):
            rows.append(candmat[i, :])
            rhs.append(residual[i])

    A = np.array(rows, dtype=float)
    b = np.array(rhs, dtype=float)
    coeffs, residuals, rank, singular = np.linalg.lstsq(A, b, rcond=None)

    print("Post-rotmix global residual fit")
    print("coefficients =", dict(zip(FOLLOWUP_CANDIDATES, coeffs)))
    print("rank =", rank)
    print("singular values =", singular)
    if residuals.size:
        print("residual sumsq =", float(residuals[0]))

    for species, (base, target, residual, candmat) in cache.items():
        fit = base + candmat @ coeffs
        print(f"\n{species.upper()}")
        print(f"  ||diag0+rotmix|| / ||target|| = {np.linalg.norm(base) / np.linalg.norm(target):.6g}")
        print(f"  ||fitted||       / ||target|| = {np.linalg.norm(fit) / np.linalg.norm(target):.6g}")
