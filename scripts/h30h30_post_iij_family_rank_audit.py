#!/usr/bin/env python3
"""Family-wise rank audit of the H30H30 residual after removing the best iij branch."""
...
def _post_iij_residual(species: str) -> np.ndarray:
    ...
    return residual - alpha * iij


def main() -> None:
    all_res = {name: _post_iij_residual(name) for name in VCI_PAPER}
    for family, species_list in FAMILIES.items():
        mat = np.column_stack([all_res[name] for name in species_list])
        s = np.linalg.svd(mat, full_matrices=False)[1]
        print(f"{family}: species={species_list}")
        print(f"  rank = {np.linalg.matrix_rank(mat)}")
        print(f"  singular values = {s}")
        if len(species_list) == 2:
            corr = float(np.dot(mat[:, 0], mat[:, 1]) / max(np.linalg.norm(mat[:, 0]) * np.linalg.norm(mat[:, 1]), 1.0e-30))
            print(f"  cosine(pair) = {corr: .6e}")
        print()
