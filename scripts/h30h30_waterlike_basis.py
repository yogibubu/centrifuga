#!/usr/bin/env python3
"""Extract a minimal water-like residual basis inside the current follow-up span."""
...
def _species_residual_and_followup(species: str):
    ...
    diag_basis = np.column_stack([vec(name) for name in DIAG_BASIS])
    target = np.array([VCI_PAPER[species][key] for key in WATSON_KEYS], dtype=float)
    coeffs, *_ = np.linalg.lstsq(diag_basis, target, rcond=None)
    residual = target - diag_basis @ coeffs
...
def main() -> None:
    ...
    names = []
    followup_ref = None
    print("H30H30 water-like residual basis inside current follow-up span")
    for species in ("h2o", "h2s"):
        residual, followup = _species_residual_and_followup(species)
        if followup_ref is None:
            followup_ref = followup
        coeffs, *_ = np.linalg.lstsq(followup, residual, rcond=None)
        residuals.append(residual)
        names.append(species.upper())
        print(f"\n{species.upper()}")
        print("  residual =", residual)
        print("  follow-up coeffs =", dict(zip(FOLLOWUP_CANDIDATES, coeffs)))
    resid_mat = np.column_stack(residuals)
    u, s, _vh = np.linalg.svd(resid_mat, full_matrices=False)
    print()
    print(f"residual rank = {np.linalg.matrix_rank(resid_mat)}")
    print(f"singular values = {s}")
    print("orthonormal Watson-space basis columns:")
    for i in range(u.shape[1]):
        print(f"  water_basis_{i+1} = {u[:, i]}")
    if followup_ref is not None:
        print()
        print("expansion of orthonormal water-like basis in current follow-up span:")
        for i in range(u.shape[1]):
            coeffs, *_ = np.linalg.lstsq(followup_ref, u[:, i], rcond=None)
            print(f"  water_basis_{i+1} coeffs = {dict(zip(FOLLOWUP_CANDIDATES, coeffs))}")
