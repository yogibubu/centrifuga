#!/usr/bin/env python3
"""Fit BCH order-2/order-3 coefficients for the manual H30H30 iij pivot."""
...
def main() -> None:
    ...
    for species, refs in VCI_PAPER.items():
        ...
        o2 = _watson_vector(species, "manual_iij_order2_candidate")
        o3 = _watson_vector(species, "manual_iij_order3_candidate")
        residual = delta - trusted
        a = np.column_stack([o2, o3])
        coeffs, *_ = np.linalg.lstsq(a, residual, rcond=None)
        post = residual - a @ coeffs
        ratio = coeffs[1] / coeffs[0] if abs(coeffs[0]) > 1.0e-30 else np.nan
        print(
            f"{species:>4}  alpha2={coeffs[0]: .6e}  alpha3={coeffs[1]: .6e}  "
            f"alpha3/alpha2={ratio: .6e}  ||post||/||delta||={np.linalg.norm(post)/np.linalg.norm(delta): .6e}"
        )
        all_a.append(a)
        all_b.append(residual)

    all_a = np.vstack(all_a)
    all_b = np.concatenate(all_b)
    coeffs, *_ = np.linalg.lstsq(all_a, all_b, rcond=None)
    post = all_b - all_a @ coeffs
    ratio = coeffs[1] / coeffs[0] if abs(coeffs[0]) > 1.0e-30 else np.nan
    print()
    print(
        f"global alpha2={coeffs[0]: .6e}  alpha3={coeffs[1]: .6e}  "
        f"alpha3/alpha2={ratio: .6e}  ||post||/||res||={np.linalg.norm(post)/np.linalg.norm(all_b): .6e}"
    )
