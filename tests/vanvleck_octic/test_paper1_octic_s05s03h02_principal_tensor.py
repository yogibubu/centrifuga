...
def test_s05s03h02_principal_tensor_is_degree8_and_orthorhombic() -> None:
...
def test_s05s03h02_sigma3_matches_watson_s111_normalization() -> None:
    out = paper1_octic_s05s03h02_principal_tensor()
    sigma3, S111 = sp.symbols("sigma3 S111")
    assert out["sigma3_to_S111"] == sp.Eq(sigma3, -sp.Rational(3, 2) * S111)


def test_s05s03h02_depends_on_three_s05_coefficients() -> None:
    out = paper1_octic_s05s03h02_principal_tensor()
    coeffs = tuple(out["orth_coeffs_S111"].values())
    assert any(sp.Symbol("s311") in coeff.free_symbols for coeff in coeffs)
    assert any(sp.Symbol("s131") in coeff.free_symbols for coeff in coeffs)
    assert any(sp.Symbol("s113") in coeff.free_symbols for coeff in coeffs)
